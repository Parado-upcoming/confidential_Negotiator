    ...
    for (const chainIdStr of fs.readdirSync(scriptPath)) {
      ...
      for (const runPath of runFiles) {
        ...
        for (const tx of run.transactions ?? []) {
          ...
          // Newest CREATE per (contract, chain) wins, reflecting current on-chain state.
 ...
 *  main files exist but the gitignored sidecars don't, and without these stubs,
...
function cleanStaleContractFiles(currentNames: Set<string>) {
  ...
  for (const file of fs.readdirSync(TARGET_DIR)) {
    ...
    // Generated files start with a JSDoc marker we can detect, so skip any
